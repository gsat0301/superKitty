...
function createPlayer() {
    // Draw player
    player = this.physics.add.sprite( /*screenWidth * 1.5*/ startOffset, screenHeight - platformHeight, 'kitty').setOrigin(1).setBounce(0)
    .setCollideWorldBounds(true).setScale(screenHeight / 375 );
    player.depth = 3;
...
function updatePlayer(delta) {
    ...
    if (!playerFiring) {
        if (playerState > 0 && (controlKeys.DOWN.isDown|| this.joyStick.down)) {
            ...
            player.body.setSize(40, 40).setOffset(2, 10);

            return;
        } else {
            if (playerState > 0)
                player.body.setSize(25, 34).setOffset(2,0);

            if (playerState == 0)
                player.body.setSize(23, 32).setOffset(1.3, 0.5);
        }
    }
